...
Radiator radiator;
//-----------------------------------------------------
Radiator::Radiator(void)
{
    isOnOld = !isOn;
    value = START_VALUE;
}
//-----------------------------------------------------
void Radiator::viewScreen(void)
{
    bool result;
    uint32_t i;
...
//-----------------------------------------------------
uint8_t Radiator::viewHandler(void)
{
    uint8_t a, result;
...
//-----------------------------------------------------
void Radiator::viewMode(void)
{
    if (isOnOld != isOn){
        isOnOld = isOn;
        if (isOn == true){
            canvas.loadImageEffect(BUTTON_RADIATOR_X,BUTTON_RADIATOR_Y,BLUE_RADIATOR_IMAGE,BUTTON_RADIATOR_STEP,0);      // отображение значка отопления
        }
        else{
            canvas.loadImageEffect(BUTTON_RADIATOR_X,BUTTON_RADIATOR_Y,BLACK_RADIATOR_IMAGE,BUTTON_RADIATOR_STEP,0);      // отображение значка отопления
        }
    }
}
//-----------------------------------------------------
void Radiator::viewTemperature(void)
{
    uint8_t x, n;
    char str[7];
    signed char val;
...
//-----------------------------------------------------
uint8_t Radiator::sensorCheck(void)
{
    uint8_t result = 0;
    int8_t s;
    uint32_t i;
    ...
    if (sensor.status == 1 && sensor.touch == 0){   // касание сенсорного экрана
        ...
        if (1){
            if (slider.touch==0 && 
                sensor.x1>(BUTTON_RADIATOR_X) && sensor.x1<(BUTTON_RADIATOR_X+BUTTON_RADIATOR_SIZE) && 
                sensor.y1>(BUTTON_RADIATOR_Y) && sensor.y1<(BUTTON_RADIATOR_Y+BUTTON_RADIATOR_SIZE)){   // касание кнопки отопления
                sensor.touch = 0;
                isOn = !isOn;
            }
            if (slider.touch==0 && 
...
//-----------------------------------------------------
void Radiator::buttonPress(void)
{
...
//-----------------------------------------------------
void Radiator::buttonCheck(uint8_t mode)
{
