...
//-----------------------------------------------------
uint8_t Water::viewHandler(void)
{
    uint8_t a, result;
...
//-----------------------------------------------------
void Water::viewMode(void)
{
    if (isOnOld != isOn){
        isOnOld = isOn;
        if (isOn == true){
            canvas.loadImageEffect(BUTTON_WATER_X,BUTTON_WATER_Y,BLUE_WATER_IMAGE,BUTTON_WATER_STEP,0);      // отображение значка хоз.воды
        }
        else{
            canvas.loadImageEffect(BUTTON_WATER_X,BUTTON_WATER_Y,BLACK_WATER_IMAGE,BUTTON_WATER_STEP,0);      // отображение значка хоз.воды
        }
    }
}
//-----------------------------------------------------
void Water::viewTemperature(void)
{
    uint8_t x, n;
    char str[7];
    signed char val;
...
//-----------------------------------------------------
uint8_t Water::sensorCheck(void)
{
    uint8_t result = 0;
    int8_t s;
    uint32_t i;
    ...
    if (sensor.status == 1 && sensor.touch == 0){   // касание сенсорного экрана
        ...
        if (1){
            if (slider.touch==0 && 
                sensor.x1>(BUTTON_WATER_X) && sensor.x1<(BUTTON_WATER_X+BUTTON_WATER_SIZE) && 
                sensor.y1>(BUTTON_WATER_Y) && sensor.y1<(BUTTON_WATER_Y+BUTTON_WATER_SIZE)){   // касание кнопки хоз.воды
                sensor.touch = 0;
                isOn = !isOn;
            }
            if (slider.touch==0 && 
...
//-----------------------------------------------------
void Water::buttonCheck(uint8_t mode)
{
